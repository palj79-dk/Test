...
- **Total Alloy sink** should stay in the ballpark of today's re-paced curve (V6.17 set ~25,700 Alloy total ≈ 45+ Normal wins). 4 branches × ~8 nodes = **~32 nodes** (see §3.6); distribute the ~25k so early unlocks are cheap (first Mortar/Tesla reachable in the first few wins) and capstones are long-tail (the aspirational + future-monetizable targets).
  - **Shipped state (V6.36):** V6.24 priced each node in isolation and the aggregate landed at **10,670** — 2.4× under this target, which a clean-account play-log confirmed: 8,476 Alloy earned by mission 9 covered 91% of the tree. **V6.36 re-costed to 25,380** (Ordnance 6,195 · Arc 6,770 · Logistics 6,005 · Command 6,410), scaling by ladder position so the opening is untouched — index 0 unchanged, index 1 +10%, rising to ×3.2 at the capstone. The first two nodes of all four branches still total 1,230, and **20,010** is reachable before the mission-12 capstone gate, which paces the 25-mission campaign. **Price nodes against the branch total, not one at a time.**
