...
- **Iter 1 — V6.0 Threat Level (dynamic scaling).** `threat = purchased talent levels / total levels` (0–1). Enemy HP ×(1 + threat×~0.85) + slight speed; **shown openly** ("⚠ THREAT LVL" in menu + HUD/briefing) and **paid** with an alloy bonus so it reads as prestige, not punishment. Fresh account (threat 0) = zero change. Applies across modes (incl. custom — orthogonal to chosen difficulty).
