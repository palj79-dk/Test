...
- Verified (Playwright, `verify50.py`): new enemies spawn in waves 8–15; **slow floor holds at exactly 0.30** and **Cinder is freeze-immune** (`slowF=1`, never slowed); **Insulator takes 40%** of a raider's Hero-Pulse (energy) damage; congestion cap holds the timer at ≥45 then resumes on clear; **balance read at Hard through wave 15: mono-Tesla keeps core 5 vs diverse core 10** — diverse overlapping builds now beat the single-type wall; no-tower→gameover, maxed diverse build→victory, campaign intact. 0 page errors, guards intact, `node -c` passes. **Next: Iter 2 · V5.1 custom-difficulty sliders.**
